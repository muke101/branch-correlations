...
def run_alberta():
    for bench in spec:
        ...
        for workload in os.listdir(workloads+stripped_name):
            ...
            p = subprocess.Popen(gem5+"build/ARM/gem5.fast --outdir="+checkpoint_path+"/checkpoints."+str(workload)+" "+gem5+"configs/deprecated/example/se.py --cpu-type=NonCachingSimpleCPU --take-simpoint-checkpoint=/work/muke/simpoints-expanded/"+bench_name+".simpts,/work/muke/simpoints-expanded/"+bench_name+".weights,100000000,10000000 -c "+binary+" --options=\""+flags+"\" --mem-size=50GB 2>&1 > "+bench_name+".out 2>&1", shell=True)
            procs.append(p)
            load_balance()
