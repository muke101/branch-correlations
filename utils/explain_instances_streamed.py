...
def run_lime(instances, branch, result_queue, device, num_features, num_samples):
    ...
    for i in range(0, len(instances), interval):
        ...
        for indx, exp in zip(indxs, exps):
            input_table = {k: pa.array([v]) for k,v in instances.row(indx, named=True).items()}
            output_table = { "explanation": pa.array([exp.as_list()], type=pa.list_(pa.struct([pa.field("feature", pa.int64()), pa.field("impact", pa.float64())]))) }
