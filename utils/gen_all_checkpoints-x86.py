    ...
    for bench in get_traces.branchnet_benchmarks:
        ...
        for workload in os.listdir(workloads+stripped_name):
            ...
            for c, line in enumerate(lines):
                ...
                if len(lines) > 1 and bench == "625.x264_s":
                    subprocess.run(gem5+"build/X86/gem5.fast --outdir="+checkpoint_path+"/checkpoints."+str(run_name)+" "+gem5+"configs/deprecated/example/se.py --cpu-type=X86KvmCPU --take-simpoint-checkpoint=/work/muke/simpoints-x86/"+bench_name+".simpts,/work/muke/simpoints-x86/"+bench_name+".weights,100000000,10000000 -c "+binary+" --options=\""+flags+"\" --mem-size=50GB 2>&1 > "+bench_name+".out 2>&1", shell=True, check=True)
                else:
                    p = subprocess.Popen(gem5+"build/X86/gem5.fast --outdir="+checkpoint_path+"/checkpoints."+str(run_name)+" "+gem5+"configs/deprecated/example/se.py --cpu-type=X86KvmCPU --take-simpoint-checkpoint=/work/muke/simpoints-x86/"+bench_name+".simpts,/work/muke/simpoints-x86/"+bench_name+".weights,100000000,10000000 -c "+binary+" --options=\""+flags+"\" --mem-size=50GB 2>&1 > "+bench_name+".out 2>&1", shell=True)
                    procs.append(p)
                load_balance()
            control.close()
