...
def get_best():
  ...
  for dir in os.listdir(base_dir):
    if dir.startswith("full_h2p_best_choices_new") and os.path.exists(os.path.join(base_dir, dir, "base", "m4", "leela.0", "results_debug.txt")):
...
if __name__ == "__main__":
  dict = get_best()
  filtered_dict = {pc: (base_acc, acc, config, transformer_acc, improvement, base_MPKI, MPKI) for pc, (base_acc, acc, config, transformer_acc, improvement, base_MPKI, MPKI) in dict.items() if config >= 0 and MPKI != 0 and improvement > 0}
  # print(dict)
  # print(filtered_dict)
  total_improvement = 0
